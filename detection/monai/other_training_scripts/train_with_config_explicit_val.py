...
def apply_cli_overrides(cfg, args):
    ...
    if args.train_image_dir is not None:
        cfg["paths"]["train_image_dir"] = args.train_image_dir
    if args.train_label_dir is not None:
        cfg["paths"]["train_label_dir"] = args.train_label_dir
    if args.val_image_dir is not None:
        cfg["paths"]["val_image_dir"] = args.val_image_dir
    if args.val_label_dir is not None:
        cfg["paths"]["val_label_dir"] = args.val_label_dir
...
def validate_config(cfg):
    ...
    for key in required_paths:
        if not cfg["paths"].get(key):
            raise ValueError(f"Missing paths.{key}. Set it in YAML or pass --{key}.")
...
def main(cfg):
    ...
    train_data = build_file_list(
        paths["train_image_dir"],
        paths["train_label_dir"],
    )
    val_data = build_file_list(
        paths["val_image_dir"],
        paths["val_label_dir"],
    )

    print("Using explicit train/validation directories; no internal split is performed.")
    print(f"Train cases: {len(train_data)}")
    print(f"Val cases: {len(val_data)}")

    wandb.config.update({
        "num_total_cases": len(train_data) + len(val_data),
...
CUDA_VISIBLE_DEVICES=0 python experiments/3d_seg/train_with_config_explicit_val.py \
    --config experiments/3d_seg/configs/synthTrainRealValFold0/synthTrainRealValFold0_light_96x96x48_pos2neg1_config.yaml

This script does not create a random or K-fold split. It directly consumes:
    paths.train_image_dir
    paths.train_label_dir
    paths.val_image_dir
    paths.val_label_dir
